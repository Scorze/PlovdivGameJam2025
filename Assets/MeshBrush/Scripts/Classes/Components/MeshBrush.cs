...
namespace MeshBrush
{
    ...
    public class MeshBrush : MonoBehaviour
    {
        ...
        [SerializeField]
        private float maxDelayLimit = 1.0f;

        [SerializeField]
        private float maxDensityLimit = 10.0f;

        [SerializeField]
        private float minOffsetLimit = -50.0f;

        [SerializeField]
        private float maxOffsetLimit = 50.0f;

        [SerializeField]
        private float maxMinimumAbsoluteDistanceLimit = 3.0f;

        [SerializeField]
        private float maxAdditiveScaleLimit = 3.0f;

        [SerializeField]
        private float maxRandomScaleLimit = 3.0f;
        ...
        public void PaintMeshes(RaycastHit brushLocation)
        {
            ...
            {
                // Calculate how far away from the center of the brush the meshes should be allowed to end up.
                ...
                (
                    globalPaintingMode 
                        ? Physics.Raycast(new Ray(brushTransform.position, -brushLocation.normal), out tempHit, 2.5f) 
                        : CachedCollider.Raycast(new Ray(brushTransform.position, -brushLocation.normal), out tempHit, 2.5f))
                {
                    // Calculate the slope angle based on the angle between the world's up-vector (or a manually sampled reference vector) and the normal vector of our hit.
                    float slopeAngle = useSlopeFilter ? Vector3.Angle(tempHit.normal, manualReferenceVectorSampling ? slopeReferenceVector : Vector3.up) : inverseSlopeFilter ? 180.0f : 0.0f;
                    ...
                    if (inverseSlopeFilter ? slopeAngle > Random.Range(angleThresholdRange.x, angleThresholdRange.y) : slopeAngle < Random.Range(angleThresholdRange.x, angleThresholdRange.y))
                    {
                        ...
                        List<GameObject> set = meshes.Except(deactivatedMeshes).ToList();
                        if (set.Count <= 0)
                        {
                        ...
                        Transform paintedMeshTransform = paintedMesh.transform;
        ...
        public void RandomizeMeshes(RaycastHit brushLocation)
        {
            ...
            {
                Transform transformToRandomize = paintedMeshesInsideBrushArea[i];
